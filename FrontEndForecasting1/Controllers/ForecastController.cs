...
namespace FrontEndForecasting.Controllers
{
    ...
    public class ForecastController : Controller
    {
        ...
        /// <summary>
        /// Test endpoint to verify Redis caching functionality
        /// </summary>
        /// <returns>JSON response with cache test results</returns>
        [HttpGet]
        public async Task<IActionResult> TestCache()
        {
            try
            {
                ...
                var testData = new { message = "Hello from Redis!", timestamp = DateTime.UtcNow };

                // Test setting cache
                await _cacheService.SetAsync(testKey, testData, TimeSpan.FromMinutes(5));
                _logger.LogInformation("Test data cached with key: {Key}", testKey);

                // Test getting cache
                var retrievedData = await _cacheService.GetAsync<object>(testKey);
